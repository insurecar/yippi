...
function isTtagImport(node) {
  return node.source.value === _defaults.TTAGID || node.source.value === _defaults.TTAG_MACRO_ID || node.source.value === _defaults.INTERNAL_TTAG_MACRO_ID;
}

function isTtagRequire(node) {
  return bt.isCallExpression(node.init) && node.init.callee.name === 'require' && bt.isObjectPattern(node.id) && node.init.arguments.length === 1 && (node.init.arguments[0].value === _defaults.TTAGID || node.init.arguments[0].value === _defaults.TTAG_MACRO_ID || node.init.arguments[0].value === _defaults.INTERNAL_TTAG_MACRO_ID);
}
